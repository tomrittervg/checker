...
class PeerChecker(JobSpawner.JobSpawner):
    class IndividualPeerChecker(JobBase.JobBase):
        def __init__(self, config, checkurl, notificationAddress):
            ...
            super(PeerChecker.IndividualPeerChecker, self).__init__(
                config, checkurl, notificationAddress
            )
        ...
        def execute(self):
            ...
            try:
                ...
                if response.status_code != 200:
                    peerOK = False
                    self.subject = (
                        self.checkurl + " returned a non-standard status code."
                    )
                    ...
                    if "True" in content:
                        ...
                        peerOK = False
                        self.subject = (
                            self.checkurl + " reports its jobs are not running."
                        )
                        self.body = str(response.status_code) + "\n" + content
                    else:
                        peerOK = False
        ...
        def onStateChangeSuccess(self):
            return self.sendEmail(
                "Successfully hit " + self.checkurl, "", self.notificationAddress
            )
